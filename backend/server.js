...
// ─── API AUTH MIDDLEWARE ───────────────────────────────────
const API_AUTH_KEY = process.env.API_AUTH_KEY || "abbi-default-key-change-me";
app.use("/api/private", (req, res, next) => {
  // Bypass auth for localhost/same-server requests
...
// Alias untuk frontend (supaya cocok dengan path yang frontend cari)
app.get('/api/indodax/ticker/:pair', async (req, res) => {
  try {
    const response = await axios.get(
      `https://indodax.com/api/ticker/${req.params.pair}`,
      { timeout: 10000 }
...
app.get('/api/indodax/ticker/:pair', async (req, res) => {
  try {
    ...
    console.error('[Indodax Ticker Error]', err.message);
    res.status(502).json({ error: err.message });
  }
});

app.get('/api/indodax/ticker_all', async (req, res) => {
  try {
    const response = await axios.get('https://indodax.com/api/ticker_all', {
      timeout: 10000,
    });
    ...
    console.error('[Indodax TickerAll Error]', err.message);
    res.status(502).json({ error: err.message });
  }
});

app.get('/api/indodax/depth/:pair', async (req, res) => {
  try {
    const response = await axios.get(
      `https://indodax.com/api/depth/${req.params.pair}`,
      { timeout: 10000 }
    ...
    console.error('[Indodax Depth Error]', err.message);
    res.status(502).json({ error: err.message });
  }
});
